...
$(document).ready( function(){
	...
	function charDisplay (){
		...
		charList.forEach(function (char, index) {
			...
			var character = $("<li class='character-list'>").attr("id", index);

			character.append($("<p class='name'>").text(char.name));

			character.append($("<img>").attr("src", char.img));

			character.append($("<p class='hp'>").text("HP : " + char.hp));

			char.$me = character;

			$("#character-container").append(char.$me);
	...
	function winFlag (){
		...
		//randon phrase
		var randomPhrase = winFlag[Math.floor(Math.random() * winFlag.length)]
		//randon relative path from 1 to 3
		var randomYodaPic = yoda + [Math.floor(Math.random() * 3)] +".png";
		
		var result = $("<li class='winFlag'>");
		result.append( $("<img class= winFlag-img>").attr("src", randomYodaPic) );
		result.append( $("<p class= winFlag-text>").text(randomPhrase) );
